...
class Baidu(BaseES):
      def __init__(self,base_url="https://www.baidu.com",filter_include=None,filter_exclude=None,*args,**kwargs):
      ...
      def do_search(self):
            self.find_element(EBy.CSS_SELECTOR ,'input[name=wd]',lambda x:x.send_keys('东宏金融'+EKeys.RETURN))
      
      def get_element_selector(self):
            return (EBy.CSS_SELECTOR, 'h3 a')
      
      def get_page_element(self,page):
            ps = self.find_elements(EBy.CSS_SELECTOR,'#page .pc')
            print(page)
            for p in ps:
                  if p.text == str(page):
                        return p
                  
class BaiduHandler(DefaultHandler):
...
if __name__ == "__main__":
      baidu = Baidu(handler=BaiduHandler(),filter_include='东宏金融',filter_exclude='诈骗,入坑,虚假')
      baidu.start()
